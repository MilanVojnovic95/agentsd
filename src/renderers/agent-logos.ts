 ...
 *
 * Ported verbatim from AgentDeck (github.com/puritysb/AgentDeck,
 * MIT © 2025 SerendipityBound). The rendered marks are the AgentDeck creature
 * language: Claude = robot, Codex = cloud prompt, OpenClaw = crayfish,
 * OpenCode = nested-square floater.
 */

import type { AgentType } from "./state-colors";
import { dimColor, agentBrandColor } from "./state-colors";
 ...
 */
export function agentLogoIcon(agent: AgentType, size = 48, opacity = 0.7, cx = 72, cy = size / 2 + 12): string {
...
export function agentLogoWatermark(agent: AgentType, _color?: string, opacity = 0.12): string {
  const brandColor = agentBrandColor(agent);
